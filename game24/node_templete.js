...
function updateDomainInHTML(domain) {
    try {
        ...
        const htmlFiles = [
            ...
            path.join(__dirname, 'specification.html'),
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 更新顶部header中的域名
                // 更精确地匹配 <div class="header-content"> 下紧接着的 <p>域名</p> 格式
                const headerDomainRegex = /(<div class="header-content"[^>]*>[\s\S]*?)<p>[^<]+<\/p>([\s\S]*?<\/div>)/g;
                htmlContent = htmlContent.replace(headerDomainRegex, (match, prefix, suffix) => {
                    return prefix + `<p>${domain}</p>` + suffix;
                });
                ...
                if (htmlContent.includes('footer-copyright') || htmlContent.includes('footer')) {
                    ...
                    const footerCopyrightRegex = /(<div class="footer-copyright">Copyright © \d{4}(?:-\d{4})?\s+)[^\s]+(\s+All rights? Reserved\.<\/div>)/gi;
                    htmlContent = htmlContent.replace(footerCopyrightRegex, (match, prefix, suffix) => {
                        modified = true;
                        return `${prefix}${domain}${suffix}`;
                    });
                    
                    // 如果上面的匹配没有成功，尝试匹配不带div标签的格式
                    if (!modified) {
                        const inlineCopyrightRegex = /(Copyright © \d{4}(?:-\d{4})?\s+)[^\s]+(\s+All rights? Reserved\.)/gi;
                        htmlContent = htmlContent.replace(inlineCopyrightRegex, (match, prefix, suffix) => {
                            modified = true;
                            return `${prefix}${domain}${suffix}`;
                        });
                    }
