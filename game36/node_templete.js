...
function readConfig() {
    try {
        ...
        if (!config.color || !config.domain || !config.color2) {
            throw new Error('配置文件中缺少 color、color2 或 domain 字段');
        }
...
function updateCSSColor(color, color2) {
    try {
        ...
        // 使用正则表达式替换 --color1 的值
        const bgRegex = /(--color1:\s*)([^;]+)/;
        const bgValue = `$1${color}`;
        
        // 使用正则表达式替换 --color2 的值
        const bg2Regex = /(--color2:\s*)([^;]+)/;
        const bg2Value = `$1${color2}`;
        ...
        if (bg2Regex.test(cssContent)) {
            cssContent = cssContent.replace(bg2Regex, bg2Value);
            modified = true;
        }
...
async function main() {
    ...
    try {
        ...
        console.log(`  次要色: ${config.color2}`);
        console.log(`  域名: ${config.domain}`);
        console.log(`  info: ${config.info}`);
        console.log(`  json: ${config.json}\n`);
        
        // 2. 更新CSS颜色
        updateCSSColor(config.color, config.color2);
        console.log('');
