...
function populateCategorySection(section, category, loadedGameDetails, sectionIndex) {
    ...
    const gamesCount = sectionIndex === 0 ? 3 : 6;
...
function createPopularGameCardHTML(game, index) {
    const detailPath = getDetailPagePath();
    const gameUrl = `${detailPath}?id=${game.id}`;
    const bgIndex = index % 3 + 1;
    const gameType = game.type || game.category || '';
    const truncatedName = game.name.length > 12 ? game.name.substring(0, 12) + '...' : game.name;
    return `
        <a href="${gameUrl}" class="popular-game-card popular-game-card-${bgIndex}">
            ...
            <div class="popular-game-type">
                ${gameType}
            </div>
            <div class="popular-game-play">
                Play Now
            </div>
        </a>
    `;
...
function populatePopularGames(loadedGameDetails) {
    ...
    popularGamesWrapper.innerHTML = popularGames
        .map((game, index) => createPopularGameCardHTML(game, index))
        .join('');
