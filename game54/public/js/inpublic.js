...
export function createGameCardHTML(game, detailPagePath) {
    ...
    const gameName = originalName.length > 23 ? originalName.substring(0, 23) + '...' : originalName;
    const gameImage = game.image || '';
    
    return `
        <div class="game-card" onclick="window.location.href='${finalDetailPath}?id=${game.id}'">
            <div class="game-card-image-wrapper">
                <img src="${gameImage}" alt="${originalName}" loading="lazy">
            </div>
...
function generateGamesList(category, gamesGrid) {
    ...
    gamesGrid.className = 'category-game-list';
    const detailPath = getDetailPagePath();
    gamesGrid.innerHTML = games
        .map(game => createCategoryGameItemHTML(game, detailPath))
        .join('');
}

function createCategoryGameItemHTML(game, detailPath) {
    const gameUrl = `${detailPath}?id=${game.id}`;
    const rating = convertRatingToStars(game.rating);
    const originalName = game.name || 'Unknown Game';
    const gameName = originalName.length > 20 ? originalName.substring(0, 20) + '...' : originalName;
    return `
        <a href="${gameUrl}" class="category-game-item" style="text-decoration: none;">
            <div class="category-game-thumbnail">
                <img src="${game.image}" alt="${originalName}">
            </div>
            <div class="category-game-content">
                <h4 class="category-game-title">${gameName}</h4>
                <p class="category-game-rating">${rating}</p>
            </div>
            <button class="category-game-play">
                Play
            </button>
        </a>
    `;
