...
function getRequiredDOMElements() {
    const elements = {
        ...
        gameDownloads: document.getElementById(DOM_IDS.GAME_DOWNLOADS),
...
function populateGameDetails(elements, game) {
    ...
    const description = game.description || game.desc || getDefaultDescription(game.name);
    elements.gameDescription.textContent = description;
    
    const downloadCount = game.downloads || game.download || Math.floor(Math.random() * 1000000);
    elements.gameDownloads.textContent = formatDownloads(downloadCount);
    
    setupPlayButtonHandler(elements.playButton, game);
}

function getDefaultDescription(gameName) {
    return `Enjoy playing ${gameName}! This is a fun and exciting game that will keep you entertained for hours. Experience amazing gameplay and challenging levels.`;
}
