    ...
    try {
      ...
      if (categories && Array.isArray(categories) && categories.length > 0) {
        categoryItemsContainer.innerHTML = categories
          .map(
            (category) => {
              const count = gameDetails.filter((game) => game.category === category).length;
              return `
                <div class="category-item" data-category="${category}">
                  <span class="category-name">${categoryNameMap[category] || category}</span>
        ...
        const newCategoryItems = categoryItemsContainer.querySelectorAll(".category-item");
        const categoriesMenu = document.getElementById("categoriesMenu");
        ...
        const homeItem = document.querySelector('.category-item[data-category="home"]');
        if (homeItem) {
...
if (document.body.classList.contains("category-page")) {
  (async () => {
    ...
    function generateGamesList(category) {
      if (!gameDetails || gameDetails.length === 0) {
        if (gamesGrid) {
          gamesGrid.innerHTML = '<div class="no-results"><h3>Loading games...</h3></div>';
        }
        return;
      }
    ...
    function displayGames(games) {
      ...
      for (let i = 0; i < games.length; i += 6) {
        const group = games.slice(i, i + 6);
        
        group.forEach((game, idx) => {
          const isLarge = idx === 4;
          gameCards.push(`
            <a href="../detail.html?id=${game.id}${
                window.channel ? "&channel=" + window.channel : ""
              }" class="game-card${isLarge ? ' large' : ''}">
              <img src="${game.image}" alt="${game.name}">
            </a>
          `);
        });

        if (i === 0 && games.length > 6) {
          gameCards.push(`
