...
// 更新CSS中的primary颜色和color2颜色
function updateCSSColors(color, color2) {
    try {
        const cssPath = path.join(__dirname, 'public', 'style', 'inpublic.css');
        let cssContent = fs.readFileSync(cssPath, 'utf8');
        
        const updateColorVariable = (name, value) => {
            const regex = new RegExp(`(--${name}:\\s*)([^;]+)`);
            if (regex.test(cssContent)) {
                cssContent = cssContent.replace(regex, `$1${value}`);
                console.log(`✓ CSS ${name} 已更新: ${value}`);
            } else {
                ...
                console.log(`✓ CSS ${name} 已添加: ${value}`);
            }
        };
        
        updateColorVariable('primary', color);
        updateColorVariable('color2', color2);
...
function updateDomainInHTML(domain) {
    try {
        ...
        htmlFiles.forEach(filePath => {
            ...
            // 3. 更新版权信息中的域名
            const copyrightRegex = /(<p>(?:Copyright\s+)?©\s*\d{4}\s+)([\w\.]+?)(<\/p>)/gi;
...
async function main() {
    ...
    try {
        ...
        // 3. 更新CSS颜色
        updateCSSColors(config.color, config.color2);
        console.log('');
