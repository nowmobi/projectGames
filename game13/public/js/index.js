...
function populateCategorySection(section, category, loadedGameDetails) {
    ...
    const shuffledCategoryGames = shuffleArray(categoryGames).slice(0, 8);
    
    if (shuffledCategoryGames.length > 0) {
        
        const categoryCardHTML = createCategoryCardHTML(category);
        
        const gamesHTML = shuffledCategoryGames.map(game => createGameCardHTML(game)).join('');
        grid.innerHTML = categoryCardHTML + gamesHTML;
    } else {
...
function displaySearchResults(games, searchTerm) {
    ...
    if (games.length === 0) {
        ...
        if (firstSection) {
            ...
            firstSection.innerHTML = `
                <div class="category-games-list" id="searchResultsGrid"></div>
            `;
            const gridContainer = firstSection.querySelector('.category-games-list');
            if (gridContainer) {
    ...
    allSections.forEach((section, index) => {
        if (index === 0) {
            ...
            container.className = 'category-games-list';
            container.id = 'searchResultsGrid';
            section.innerHTML = '';
            section.appendChild(container);
            
            
            container.innerHTML = games.map(game => createGameCardHTML(game, undefined, true)).join('');
        } else {
            section.style.display = 'none';
        }
    });
...
function createCategoryCardHTML(category) {
    ...
    return `
        <div class="game-card category-icon-card" onclick="window.location.href='${categoryPagePath}'">
            <div class="game-card-image-wrapper">
                <img src="${iconUrl}" alt="${categoryName}" loading="lazy">
                <div class="category-arrow-overlay">
                    <svg width="40" height="18" viewBox="0 0 40 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 0 L10 9 L0 18" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M10 0 L20 9 L10 18" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
            </div>
        </div>
