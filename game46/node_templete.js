    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 1. 更新页面顶部的域名
                // 匹配header-center中的p标签内容（所有页面顶部的域名位置）
                const headerRegex = /(<div[^>]*class=["']header-center["'][^>]*>[\s\S]*?<p>)[^<]+(<\/p>[\s\S]*?<\/div>)/g;
                htmlContent = htmlContent.replace(headerRegex, (match, prefix, suffix) => {
                    modified = true;
                    return `${prefix}${displayDomain}${suffix}`;
                });
                
                // 同时处理带有logo类的链接中的域名（index.html的特殊情况）
                const logoRegex = /(<a[^>]*class=["']logo["'][^>]*>[\s\S]*?<p>)[^<]+(<\/p>[\s\S]*?<\/a>)/gi;
                htmlContent = htmlContent.replace(logoRegex, (match, prefix, suffix) => {
                    modified = true;
                    return `${prefix}${displayDomain}${suffix}`;
                });
                ...
                // 4. 匹配 "Copyright © 2021-2026 域名. All rights Reserved." 格式
...
function updateBaseURL(info, json) {
    try {
        ...
        const infoTypeRegex = /return\s*['"]info(\d+)['"]\s*;/;
