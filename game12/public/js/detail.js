...
import { createGameCardHTML, getUrlParameter, getHomePagePath, displayEmptyResults } from './inpublic.js';
...
const DOM_IDS = {
    ...
    GAME_DESCRIPTION_TITLE: 'gameDescriptionTitle',
...
const RECOMMENDED_GAMES_COUNT = 16;
...
function getRequiredDOMElements() {
    const elements = {
        ...
        gameDescriptionTitle: document.getElementById(DOM_IDS.GAME_DESCRIPTION_TITLE),
        playButton: document.getElementById(DOM_IDS.PLAY_BUTTON)
    };
    
    const requiredElements = ['gameIcon', 'gameTitle', 'gameCategory', 'gameDescription', 'gameDescriptionTitle', 'playButton'];
...
function populateGameDetails(elements, game) {
    ...
    if (elements.gameRating) {
        const ratingHTML = createRatingHTML(game.rating);
        if (ratingHTML) {
            elements.gameRating.innerHTML = ratingHTML;
        } else {
            elements.gameRating.style.display = 'none';
        }
    }
    
    const categoryDisplayName = game.category ? game.category.charAt(0).toUpperCase() + game.category.slice(1) : 'Game';
    elements.gameCategory.textContent = categoryDisplayName;
    
    if (elements.gameDescriptionTitle) {
        elements.gameDescriptionTitle.textContent = game.name;
    }
