...
function updateDomainInHTML(domain) {
    try {
        ...
        const htmlFiles = [
            ...
            path.join(__dirname, 'secondary.html'),
...
function updateInfoType(info) {
    try {
        ...
        if (infoTypeRegex.test(jsContent)) {
            ...
            fs.writeFileSync(baseURLPath, jsContent, 'utf8');
            console.log(`✓ BaseURL.js 中的 infoType 已更新: info${info}`);
        } else {
            console.log(`○ BaseURL.js 中未找到 infoType 或格式不匹配`);
        }
    } catch (error) {
        console.error('更新 BaseURL.js 中的 infoType 失败:', error.message);
        throw error;
    }
}
