...
import { createGameCardHTML, createGameCardHTMLWithoutTitle, createGameListItemHTML, getCategoryDisplayName, getCategoryPagePath, getDetailPagePath, displayEmptyResults } from './inpublic.js';
...
function populateCategorySection(section, category, loadedGameDetails, sectionIndex) {
    const isListStyle = false; // 所有section都使用网格样式
    const isOddSection = true; // 所有section都显示9个游戏
    const gamesCount = 9; // 所有section都显示9个游戏
        ...
        if (index === 0) {
            ...
            section.innerHTML = `
                ...
                <div class="game-grid game-grid-three-columns">
                    ${games.map(game => createGameCardHTMLWithoutTitle(game)).join('')}
                </div>
            `;
        } else {
            section.style.display = 'none';
