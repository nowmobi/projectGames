import { loadGameData, getGameDetail, getGameUrl, getCategoryOrder } from './BaseURL.js';
import { getUrlParameter } from './inpublic.js';

let gameDetails = [];
let gameDetailContainer = null;
let backBtn = null;
let menuToggle = null;
let closeCategories = null;
...
function generateSidebarCategories(categories) {
    ...
    categories.forEach((category) => {
        ...
        const categoryName = categoryInfo[categoryLower]?.name || category.charAt(0).toUpperCase() + category.slice(1);
...
function generateRecommendedGames(currentGameId) {    console.log('generateRecommendedGames called, gameDetails length:', gameDetails ? gameDetails.length : 'null');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const [gameData, categoryOrder] = await Promise.all([
            loadGameData(),
            getCategoryOrder()
        ]);
