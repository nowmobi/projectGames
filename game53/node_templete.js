...
function updateCSSColor(color) {
    try {
        ...
        // 更新主颜色 --primary-color
        const primaryColorRegex = /(--primary-color\s*:\s*)([^;]+)(;)/;
        if (primaryColorRegex.test(cssContent)) {
            cssContent = cssContent.replace(primaryColorRegex, `$1${color}$3`);
        } else {
            // 在 :root 块内添加 --primary-color
            cssContent = cssContent.replace(/(:root\s*\{)/, `$1\n    --primary-color: ${color};`);
        }
        
        // 如果是十六进制颜色，更新 --primary-color-rgb
        if (color.startsWith('#')) {
            try {
                const rgb = hexToRgb(color);
                const rgbValue = `${rgb.r}, ${rgb.g}, ${rgb.b}`;
                
                // 更新 --primary-color-rgb（只修改这两个变量）
                const rgbVarRegex = /(--primary-color-rgb\s*:\s*)([^;]+)(;)/;
                if (rgbVarRegex.test(cssContent)) {
                    cssContent = cssContent.replace(rgbVarRegex, `$1${rgbValue}$3`);
                } else {
                    // 在 --primary-color 之后添加 --primary-color-rgb
                    cssContent = cssContent.replace(
                        /(--primary-color\s*:\s*[^;]+;)/,
                        `$1\n    --primary-color-rgb: ${rgbValue};`
                    );
                }
                ...
                if (textPrimaryRegex.test(cssContent)) {
                    ...
                    // 在 --primary-color-rgb 之后添加 --text-primary
                    cssContent = cssContent.replace(
                        /(--primary-color-rgb\s*:\s*[^;]+;)/,
                ...
                console.log(`  RGB值: ${rgbValue}`);
                console.log(`  文本颜色: ${textColor}`);
            } catch (e) {
                console.warn(`无法计算颜色变量: ${e.message}`);
            }
        }
