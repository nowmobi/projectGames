...
function getRequiredDOMElements() {
    const elements = {
        ...
        gameTags: document.getElementById(DOM_IDS.GAME_TAGS),
    ...
    const requiredElements = ['gameIcon', 'gameTitle', 'gameTags', 'gameDescription', 'playButton'];
...
function populateGameDetails(elements, game) {
    ...
    if (elements.gameTags) {
        let tags = [];
        if (game.tags && Array.isArray(game.tags)) {
            tags = game.tags;
        } else if (game.tags && typeof game.tags === 'string') {
            tags = game.tags.split(',').map(c => c.trim());
        } else if (game.category) {
            tags = game.category.split(',').map(c => c.trim());
        } else {
            tags = ['Game'];
        }
        ...
        elements.gameTags.innerHTML = tagsHTML;
    }
