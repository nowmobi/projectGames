...
const gameDownload = document.getElementById("gameDownload");
...
function displayGameDetail(game) {
  ...
  if (gameDownload) {
    gameDownload.innerHTML = `<span class="download-icon"></span><span>${game.downloads || '0'}</span>`;
  }
  
  if (tagsContainer) {
    const tags = game.tags || game.tag || [];
    if (Array.isArray(tags) && tags.length > 0) {
      tagsContainer.innerHTML = tags.map(tag => `<span class="tag-item">${tag}</span>`).join("");
    } else {
      tagsContainer.innerHTML = "<span style='color: #999; font-style: italic;'>No tags available</span>";
    }
  }
...
function generateRecommendedGames(currentGame) {
  ...
  const recommended = shuffledGames.slice(0, 12);
