...
function createGameCardHTML(game, isFourthCard = false) {
...
async function generateCategoryGames(category, gridElement, limit = 3) {
    ...
    // 为每个游戏生成HTML，第4个游戏使用特殊布局
    const gamesHTML = selectedGames.map((game, index) => {
        const isFourthCard = index === 3; // 第4个游戏（索引3）
        return createGameCardHTML(game, isFourthCard);
    }).join('');
    
    gridElement.innerHTML = gamesHTML;
}
...
async function generateHomepageCategorySections() {
    ...
    for (let i = 0; i < existingSections.length && i < categories.length; i++) {
        ...
        if (gridElement) {
            await generateCategoryGames(category, gridElement, 4);
        }
    }
