...
function displaySearchResults(games) {
    ...
    // Get both game grids
    const firstGameGrid = document.getElementById('homepageGameGrid');
    
    // Get second game grid
    let secondGameGrid = null;
    if (firstGameGrid && firstGameGrid.parentElement) {
        // Get all game grids in the parent section
        const gameGrids = firstGameGrid.parentElement.querySelectorAll('.game-grid');
        if (gameGrids.length > 1) {
            secondGameGrid = gameGrids[1];
        }
    }
    
    if (games.length === 0) {
        if (firstGameGrid) {
            firstGameGrid.innerHTML = '<div class="no-results"><h3>No games found</h3><p>Try searching with different keywords</p></div>';
        }
        if (secondGameGrid) {
            secondGameGrid.innerHTML = '';
        }
        return;
    }

    // Split games between two grids
    if (firstGameGrid) {
        firstGameGrid.innerHTML = games.slice(0, 2).map(game => generateGameCardHTML(game)).join('');
    }
    if (secondGameGrid) {
        secondGameGrid.innerHTML = games.slice(2, 4).map(game => generateGameCardHTML(game)).join('');
    }
}
...
function generateHomepageGames() {
    ...
    if (firstGameGrid) {
        // Add first 2 games to the first grid
        firstGameGrid.innerHTML = selectedGames.slice(0, 2).map(game => generateGameCardHTML(game, false)).join('');
    }
    
    if (secondGameGrid) {
        // Add last 2 games to the second grid
        secondGameGrid.innerHTML = selectedGames.slice(2, 4).map(game => generateGameCardHTML(game, false)).join('');
    }
}
