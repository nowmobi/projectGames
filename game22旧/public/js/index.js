import { loadGameData, categoryIcons, getCategoryName } from './BaseURL.js';

async function renderCategoryButtons() {
  try {
    const { INFO_TYPE, getCategoryData } = await import('./BaseURL.js');
    const infoType = INFO_TYPE;
    
    const categoryData = await getCategoryData();
    
    let menuItems = null;
    if (categoryData && categoryData[infoType]) {
      menuItems = categoryData[infoType];
...
function generateGameCard(game, baseUrl) {
  return `
          <a href="particulars.html?id=${game.id}${
...
async function renderCategoryGames() {
  try {
    ...
    if (gameDetails && gameDetails.length > 0) {
      ...
      // 填充已存在的 category-games-section 容器
      const defaultCategories = ["action", "racing", "puzzle", "adventure", "sports", "kids"];
      ...
      if (categoryGamesContainer) {
        ...
        sections.forEach((section, index) => {
          if (index < defaultCategories.length) {
            ...
            // 如果 section 内没有 featured-games，则创建它
            ...
            const gamesCardsHTML = games.map(game => generateGameCard(game, baseUrl)).join('');
