    ...
    try {
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 匹配格式: <span class="logo-text">sec.bjllz.top</span> 或 <p>play.hawsers.fun</p> in logo div
                if (htmlContent.includes('logo')) {
                    // 更新 <span class="logo-text">内容</span> 格式
                    const logoSpanRegex = /(<span\s+class=["']logo-text["']>)([^<]+)(<\/span>)/g;
                    const newLogoSpanText = `$1${displayDomain}$3`;
                    
                    const originalContent = htmlContent;
                    htmlContent = htmlContent.replace(logoSpanRegex, newLogoSpanText);
                    
                    if (htmlContent !== originalContent) {
                        modified = true;
                    }
                    
                    // 更新 <div class="logo"><p>内容</p></div> 格式
                    const logoPRegex = /(<div\s+class=["']logo["']>\s*<p>)([^<]+)(<\/p>)/g;
                    const newLogoPText = `$1${displayDomain}$3`;
                    
                    const originalContent2 = htmlContent;
                    htmlContent = htmlContent.replace(logoPRegex, newLogoPText);
