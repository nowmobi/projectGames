...
function populateCategorySection(section, category, loadedGameDetails) {
    ...
    const gridClass = grid.classList.contains('game-grid-four-columns') 
        ? 'game-grid-four-columns' 
        : 'game-grid-three-columns';
    grid.classList.add(gridClass);
    
    // 使用响应式混合尺寸布局，让图片大小不一但保持良好的视觉效果
    grid.classList.add('game-layout-responsive-mixed');
    
    const categoryGames = loadedGameDetails.filter(game => game.category === category);
    const shuffledCategoryGames = shuffleArray(categoryGames).slice(0, 4); // 只取前4个游戏，删除后4张图片
    
    // 删除最后一张图片
    if (shuffledCategoryGames.length > 0) {
        shuffledCategoryGames.pop(); // 删除数组的最后一项，即最后一张图片
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Existing initialization code can be added here if needed
    initBackToTop();
});
