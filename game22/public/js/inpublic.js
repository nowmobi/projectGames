...
class GameSquareApp {
  ...
  async displaySearchResults(results) {
    ...
    const resultsHtml = results
      .map(
        (game) => `
            <div class="search-result-item" onclick="window.location.href='${pathPrefix}particulars.html?id=${
          game.id
...
function generateCategoryGameCard(game, baseUrl) {
    return `
        <a href="../particulars.html?id=${game.id}${window.channel ? '&channel=' + window.channel : ''}" class="game-card-link">
            <div class="game-card">
...
async function renderGamesList(games) {
    ...
    if (gamesList) {
        const { getDataBaseUrl } = await import('./BaseURL.js');
        const baseUrl = getDataBaseUrl();
        gamesList.innerHTML = games.map(game => generateCategoryGameCard(game, baseUrl)).join('');
    }
}
