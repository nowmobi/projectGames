...
function updateDomainInHTML(domain) {
    try {
        ...
        const htmlFiles = [
            ...
            path.join(__dirname, 'particulars.html'),
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 匹配格式1: <span class="logo-text">sec.bjllz.top</span>
                // 匹配格式2: <div class="logo"><p>kkk.topp</p></div>（同一行或多行）
                // 处理格式1: <span class="logo-text">sec.bjllz.top</span>
                if (htmlContent.includes('logo-text')) {
                    const logoRegex1 = /(<span\s+class=["']logo-text["']>)([^<]+)(<\/span>)/g;
                    const newLogoText1 = `$1${displayDomain}$3`;
                    
                    const originalContent1 = htmlContent;
                    htmlContent = htmlContent.replace(logoRegex1, newLogoText1);
                    
                    if (htmlContent !== originalContent1) {
                        modified = true;
                    }
                }
                
                // 处理格式2: <div class="logo"> 和 <p> 标签（支持多行）
                if (htmlContent.includes('logo') && htmlContent.includes('<p>')) {
                    // 通用正则表达式，支持同一行或多行格式
                    const logoRegex2 = /(<div\s+class=["']logo["']>)(\s*[\r\n]*)\s*<p>([^<]+)<\/p>(\s*[\r\n]*)\s*(<\/div>)/;
                    const newLogoText2 = `$1<p>${displayDomain}</p>$5`;
                    
                    const originalContent2 = htmlContent;
                    htmlContent = htmlContent.replace(logoRegex2, newLogoText2);
                    
                    if (htmlContent !== originalContent2) {
                        modified = true;
                    }
                }
...
function updateBaseURL(info, json, domain) {
    try {
        ...
        // 修改 INFO_TYPE
        const infoTypeRegex = /(export\s+const\s+INFO_TYPE\s*=\s*["'])([^"']+)(["'])/;
        if (infoTypeRegex.test(jsContent)) {
            const newInfoType = `info${info}`;
            // 更新lines数组中的INFO_TYPE行
            for (let i = 0; i < lines.length; i++) {
                if (infoTypeRegex.test(lines[i])) {
                    lines[i] = lines[i].replace(infoTypeRegex, `$1${newInfoType}$3`);
                    break;
                }
            }
            hasChanges = true;
            console.log(`  ✓ BaseURL.js 中的 INFO_TYPE 已更新为: ${newInfoType}`);
        }
