...
function populateCategorySection(section, category, loadedGameDetails) {
    ...
    const shuffledCategoryGames = shuffleArray(categoryGames).slice(0, 2);
...
async function initHomePage() {
    ...
    try {
        ...
        // 生成游戏统计模块
        generateGameStats(loadedGameDetails);
...
// 新增：生成游戏统计模块
function generateGameStats(gameDetails) {
    const statsContainer = document.getElementById('statsContainer');
    if (!statsContainer || !gameDetails || gameDetails.length === 0) {
        return;
    }
    ...
    // 计算平均评分
    const avgRating = gameDetails.reduce((sum, game) => {
        const rating = parseFloat(game.rating) || 0;
        return sum + rating;
    }, 0) / totalGames;
    
    // 统计总下载量（估算）
    const totalDownloads = gameDetails.reduce((sum, game) => {
        const downloads = parseDownloadsValue(game.downloads);
    ...
    const statsHTML = `
        ...
        <div class="stat-card">
            <div class="stat-icon">📥</div>
            <div class="stat-value">${formatDownloadCount(totalDownloads)}</div>
...
// 辅助函数：解析下载量数值
function parseDownloadsValue(downloadsStr) {
