...
export async function initDetailCategoryNav() {
    ...
    const navToggle = document.getElementById('categoryNavToggle');
    const categoriesMenu = document.getElementById('categoriesMenu');
    const closeCategories = document.getElementById('closeCategories');

    if (!navToggle || !categoriesMenu || !closeCategories) {
        return;
    }
    ...
    categories.forEach((category) => {
        ...
        const categoryName = (categoryInfo[categoryLower] && categoryInfo[categoryLower].name) ||
    ...
    document.addEventListener('click', (e) => {
        if (!categoriesMenu.contains(e.target) && !navToggle.contains(e.target)) {
            categoriesMenu.classList.remove('active');
        }
    });
}
