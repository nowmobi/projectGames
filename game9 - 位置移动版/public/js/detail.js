...
import { createGameCardHTML, getUrlParameter, getHomePagePath } from './inpublic.js';
...
function createModal({ className, contentHTML, onClose }) {
...
function getRequiredDOMElements() {
    const elements = {
        ...
        ratingText: document.getElementById(DOM_IDS.RATING_TEXT),
...
function populateGameDetails(elements, game) {
    ...
    elements.ratingText.textContent = `${game.rating || '0'}/5`;
    ...
    if (recommended.length === 0) {
        recommendedGamesContainer.innerHTML = `
            <div class="no-results">
                <h3>${TEXT.NO_RECOMMENDED_GAMES_TITLE}</h3>
                <p>${TEXT.NO_RECOMMENDED_GAMES_MESSAGE}</p>
            </div>
        `;
        return;
    }
