...
export function createGameCardHTML(game, detailPagePath) {
    ...
    const categoryName = getCategoryDisplayName(game.category);
    ...
    return `
        <div class="game-card" onclick="window.location.href='${finalDetailPath}?id=${game.id}'">
            <div class="game-card-image-wrapper">
                ...
                <div class="game-card-badge">${categoryName}</div>
            </div>
            <div class="game-card-content">
                <h3 class="game-card-title">${gameName}</h3>
                <div class="game-card-actions">
                    <div class="game-card-buttons">
                        <button class="game-card-btn" type="button">${gameRating}/5</button>
                        <button class="game-card-btn" type="button">${gameDownloads}</button>
                    </div>
                    <span class="game-card-link"></span>
                </div>
            </div>
        </div>
    `;
...
export function fillGameCardElement(cardElement, game, detailPagePath) {
    if (!cardElement || !game || !game.id) {
        console.warn('Invalid card element or game object:', cardElement, game);
        return;
    }
    ...
    cardElement.onclick = () => {
        window.location.href = `${finalDetailPath}?id=${game.id}`;
    };
