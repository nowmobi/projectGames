...
function populateCategorySection(section, category, loadedGameDetails, gameCount = 4) {
    ...
    let categoryGames = loadedGameDetails.filter(game => game.category === category);
    
    if (categoryGames.length < gameCount) {
        const otherGames = loadedGameDetails.filter(game => game.category !== category);
        const shuffledOtherGames = shuffleArray(otherGames);
        const neededCount = gameCount - categoryGames.length;
        categoryGames = categoryGames.concat(shuffledOtherGames.slice(0, neededCount));
    }
...
async function generateCategorySections(loadedGameDetails, forceRegenerate = false) {
    ...
    try {
        ...
        categories.forEach((category, index) => {
            ...
            const gameCount = (index + 1) % 2 === 1 ? 3 : 4;
            populateCategorySection(section, category, loadedGameDetails, gameCount);
        });
