...
import { createGameCardHTML, getCategoryDisplayName, getCategoryPagePath, displayEmptyResults, getCategoryIcon, formatGameRating, shuffleArray } from './inpublic.js';
...
let categoryAutoScrollTimer = null;
...
function initCategoryBarAutoScroll() {
    ...
    const buttonsWrap = scrollContainer.querySelector('.category-buttons');
    if (!buttonsWrap) return;

   
    if (!buttonsWrap.dataset.cloned) {
        buttonsWrap.innerHTML += buttonsWrap.innerHTML;
        buttonsWrap.dataset.cloned = 'true';
    }

    const singleWidth = buttonsWrap.scrollWidth / 2;

    const stop = () => {
        if (categoryAutoScrollTimer) {
            clearInterval(categoryAutoScrollTimer);
            categoryAutoScrollTimer = null;
        }
    };
    ...
    const start = () => {
        ...
        categoryAutoScrollTimer = setInterval(() => {
            if (scrollContainer.scrollWidth <= scrollContainer.clientWidth) return;
    ...
    scrollContainer.addEventListener('touchend', start, { passive: true });
}
...
function updateSearchIcon(hasContent) {
    ...
    if (hasContent) {
        ...
        searchIconBtn.innerHTML = `
            <img src="public/images/seach.png" alt="search" class="search-icon-img">
        `;
    }
}
