...
import { createGameCardHTML, getUrlParameter, getHomePagePath, displayEmptyResults, shuffleArray, isValidGameId, formatGameRating } from './inpublic.js';
...
function getRequiredDOMElements() {
    const elements = {
        ...
        gameIntro: document.getElementById(DOM_IDS.GAME_INTRO),
        purchaseTitle: document.getElementById(DOM_IDS.PURCHASE_TITLE),
        ...
        typeValue: document.getElementById(DOM_IDS.TYPE_VALUE),
        typeLabel: document.getElementById(DOM_IDS.TYPE_LABEL)
    };
    
    const requiredElements = ['gameIcon', 'gameIntro', 'playButton'];
...
// 使用 inpublic.js 中的 formatGameRating 替代重复的 formatRating 函数
// formatPlayersCount 和 formatAgeRating 已简化为常量
const DEFAULT_PLAYERS_COUNT = '100M+';
const DEFAULT_AGE_RATING = '12+';
...
function populateGameDetails(elements, game) {
    ...
    // 更新header标题
    const headerTitle = document.querySelector('.header-center .center-logo');
    if (headerTitle && headerTitle.parentElement) {
        headerTitle.parentElement.innerHTML = `<span class="header-title-text">${game.name}</span>`;
    }
    
    // 游戏介绍文字
    if (elements.gameIntro) {
        const introText = game.description || `嗨!我是${game.name}!一款精彩的游戏，快来体验吧!`;
        elements.gameIntro.innerHTML = `<p class="game-intro-text">${introText}</p>`;
    }
    
    // 购买区域标题
    if (elements.purchaseTitle) {
        elements.purchaseTitle.textContent = game.name || 'Game Title';
    }
    
    // 评分
    if (elements.ratingValue) {
        const rating = formatGameRating(game.rating);
        elements.ratingValue.textContent = rating.toFixed(1);
    }
    
    if (elements.ratingLabel) {
        elements.ratingLabel.textContent = 'Rating';
    }
    
    // 玩家数
    if (elements.playersValue) {
        elements.playersValue.textContent = DEFAULT_PLAYERS_COUNT;
    }
    ...
    if (elements.ageValue) {
        elements.ageValue.textContent = DEFAULT_AGE_RATING;
    }
    
    if (elements.ageLabel) {
        elements.ageLabel.textContent = 'Age';
    }
    ...
    if (elements.typeValue) {
        ...
        const typeDisplayName = firstCategory.charAt(0).toUpperCase() + firstCategory.slice(1);
        elements.typeValue.textContent = typeDisplayName;
    }
    
    if (elements.typeLabel) {
        elements.typeLabel.textContent = 'Type';
    }
