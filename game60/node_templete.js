       ...
       const htmlFiles = [
        ...
        htmlFiles.forEach(filePath => {
            if (fs.existsSync(filePath)) {
                ...
                // 1. 更新header-content中的域名（顶部域名）
                const headerContentRegex = /(<div class="header-content">[\s\S]*?<p>)([^<]+)(<\/p>[\s\S]*?<\/div>)/g;
                htmlContent = htmlContent.replace(headerContentRegex, (match, openTag, oldDomain, closeTag) => {
                    modified = true;
                ...
                // 3. 更新footer-brand域名（新结构：Welcome to xxx）
                const brandRegex = /(<div class="footer-brand">Welcome to )([\w\.]+)(<\/div>)/g;
