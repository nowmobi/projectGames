...
export async function initCategoryPage() {
    ...
    function loadCategoryPage() {
        ...
        if (category === 'recommended') {
            if (categoryTitle) {
                categoryTitle.innerHTML = '<span>Recommend</span>';
                categoryTitle.setAttribute('data-category', 'recommended');
            }
        ...
        if (categoryTitle) {
            categoryTitle.innerHTML = '<span>' + info.name.replace(' Games', ' game') + '</span>';
    ...
    function displayGames(games) {
        ...
        const gamesHTML = games.map((game, index) => {
            const displayName = game.name.length > 12 ? game.name.substring(0, 10) + '...' : game.name;
            let html = `
