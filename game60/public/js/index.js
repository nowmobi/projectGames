...
function generateGameCardHTML(game, featured = false) {
    const cardClass = featured ? 'homepage-game-card homepage-game-card-featured' : 'homepage-game-card';
    const displayName = featured ? (game.name.length > 24 ? game.name.substring(0, 24) + '...' : game.name) : (game.name.length > 12 ? game.name.substring(0, 12) + '...' : game.name);
    
    if (featured) {
        return `
            <a href="detail.html?id=${game.id}${window.channel ? '&channel=' + window.channel : ''}" class="${cardClass}">
                <div class="homepage-game-featured-big-image">
                    <img src="${game.image}" alt="${game.name}">
                </div>
                <div class="homepage-game-featured-content">
                    <div class="homepage-game-featured-small-image">
                        <img src="${game.image}" alt="${game.name}">
                    </div>
                    <div class="homepage-game-featured-info">
                        <p class="homepage-game-title">${displayName}</p>
                    </div>
                    <button class="homepage-game-featured-play-btn" onclick="event.preventDefault(); window.location.href='detail.html?id=${game.id}${window.channel ? '&channel=' + window.channel : ''}'">
                        ▶
                    </button>
                </div>
            </a>
    ...
    return `
        <a href="detail.html?id=${game.id}${window.channel ? '&channel=' + window.channel : ''}" class="${cardClass}">
            <div class="homepage-game-image-wrapper">
                <img src="${game.image}" alt="${game.name}">
            </div>
...
function generateCategoryGames(category, gridId) {
    ...
    if (info) {
        ...
        if (headerContainer) {
            headerContainer.innerHTML = `
                <h2 class="section-title" data-category="${category}"><span>${info.name}</span></h2>
            `;
        }
    }
    ...
    const featuredContainer = section.querySelector('.featured-game-container');
    if (featuredContainer && games.length > 0) {
        featuredContainer.innerHTML = generateGameCardHTML(games[0], true);
    }
    
    const remainingGames = games.slice(1);
    grid.innerHTML = remainingGames.map(game => generateGameCardHTML(game, false)).join('');
}
...
function generateRecommendedGames() {
    ...
    const recommendedGames = shuffledGames.slice(0, 7);
    
    recommendedGamesGrid.innerHTML = recommendedGames.map((game, index) => generateGameCardHTML(game, index === 0)).join('');
}
...
function generateCategorySectionContent(category, categoryInfo) {
    ...
    return `
        <div class="section-header">
            <h2 class="section-title" data-category="${categoryLower}"><span>${info.name}</span></h2>
        </div>
